...
def build_segments(token_ranges, segment_lengths, epsilon=1e-3, relay_options=None):
    ...
    KOOLOOK PATCH (Koolook issue #177, 2026-05-28):
        sigma is now computed per-segment using the Prompt-Relay paper formula
            sigma = (L - w_eff) / (2 * sqrt(ln(1/epsilon)))
        so the penalty reaches threshold epsilon exactly at the segment
        boundary regardless of segment length. The original code used a
        length-independent sigma = 1/ln(1/epsilon) (~ 0.1448 at epsilon=0.001)
        which produces a razor-sharp boundary that does not scale with L.
        SIGMA_FALLBACK preserves the prior constant for the degenerate
        L <= w_eff corner case.
    """
    SIGMA_FALLBACK = 0.1448  # = 1/ln(1000); preserves old default when L<=w_eff
    ...
    # Paper denominator: 2 * sqrt(ln(1/epsilon)), computed once per stream.
