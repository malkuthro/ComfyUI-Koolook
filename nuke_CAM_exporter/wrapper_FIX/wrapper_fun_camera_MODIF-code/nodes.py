...
class WanVideoFunCameraEmbeds:
    ...
    def process(self, poses, width, height, strength, start_percent, end_percent, fun_ref_image=None):
        ...
        target_tokens = 21  # Wan 14B emits 21 tokens per chunk in Lightning/3-step configs
        actual_tokens = control_camera_latents.shape[2]  # e.g. 41
        if actual_tokens != target_tokens:
            if actual_tokens % target_tokens != 0:
                raise ValueError(f"Cannot align {actual_tokens} camera tokens to {target_tokens}")
            group = actual_tokens // target_tokens
            control_camera_latents = (
                control_camera_latents
                .reshape(b, target_tokens, group, h, w)
