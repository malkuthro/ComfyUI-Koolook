...
def extract(wf):
    ...
    # Pull two named Text Multiline nodes from the workflow:
    #   - "OVERLAY - INFO" (or any title containing INFO) -> base notes (Δ from baseline)
    #   - "OVERLAY - FEEDBACK" / "FEEDBACK" / "OBSERVATIONS" -> video feedback
    note, feedback = "", ""
    for n in wf["nodes"]:
...
def render(data: dict, out_path: Path) -> None:
    ...
    # OUTCOME sublabel + scores
    draw.text((cx, cy), "OUTCOME", font=F_TAG, fill=ACCENT_OUT)
    cy += 20
    draw.text((cx, cy), "Motion ?/5     Sync ?/5     Sharp ?/5",
...
def main():
    if len(sys.argv) < 2:
        print("usage: make_card.py <workflow.json> [output.png]")
        sys.exit(1)
    wf_path = Path(sys.argv[1])
    out_path = Path(sys.argv[2]) if len(sys.argv) > 2 else wf_path.with_name(wf_path.stem + "_card.png")
    wf = load_workflow(wf_path)
    data = extract(wf)
    data["json_name"] = wf_path.name
    data["json_stem"] = wf_path.stem

    # Post-render extras: render duration + output filename, derived from the
    # newest video in the same folder as the JSON. mtime(video) - mtime(json)
    # is a good proxy for "time since this workflow was saved" ~= render time.
    ...
    if video is not None:
        ...
        if delta >= 0:
            ...
            data["render_duration"] = "pending (no render newer than JSON)"
        data["render_output"] = video.name
    else:
        data["render_duration"] = "pending"
        data["render_output"] = "(no video in folder yet)"
