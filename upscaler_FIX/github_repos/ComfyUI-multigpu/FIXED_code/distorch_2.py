...
def _block_fields(block):
    """Safely unpack a block tuple regardless of extra fields."""
    module_size = block[0] if len(block) > 0 else 0
    module_name = block[1] if len(block) > 1 else ""
    module_object = block[2] if len(block) > 2 else None
    params = block[3] if len(block) > 3 else None
...
def register_patched_safetensor_modelpatcher():
    ...
    if not hasattr(comfy.model_patcher.ModelPatcher, '_distorch_patched'):
        ...
        def new_partially_load(self, device_to, extra_memory=0, full_load=False, force_patch_weights=False, **kwargs):
            ...
            for block in loading:
                module_size, module_name, module_object, params = _block_fields(block)
...
def analyze_safetensor_loading(model_patcher, allocations_string, is_clip=False):
    ...
    total_memory = sum(_block_fields(block)[0] for block in raw_block_list)
    ...
    for block in distributable_raw:
        module_size, module_name, module_object, params = _block_fields(block)
...
def calculate_fraction_from_byte_expert_string(model_patcher, byte_str):
    ...
    raw_block_list = model_patcher._load_list()
    total_model_memory = sum(_block_fields(block)[0] for block in raw_block_list)
