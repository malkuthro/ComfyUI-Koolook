...
def vace_cond_execute(positive, negative, vae, width, height, length, batch_size, strength, input_video=None, control_video=None, control_masks=None, reference_image=None, latent_strength_list=None):
    ...
    if (mask_h % vae_stride) != 0 or (mask_w % vae_stride) != 0:
        raise ValueError(f"VACE mask dimensions must be divisible by {vae_stride}: got {mask_h}x{mask_w}")
    height_mask = mask_h // vae_stride
    width_mask = mask_w // vae_stride
    ...
    def upscale_video(self, model, width_upscale, height_upscale, width, height, length, pad_mask_limit, crossfade_frame, loopback_crossfade, 
        ...
        for turn in temporalist:
            ...
            for item in croparea_list:
                ...
                conditions = vace_cond_execute(
                    positive, negative, vae, width_crop_n, height_crop_n, length_n, 1, strength,
                    input_video=video, control_video=controls, control_masks=mask_ctl,
                    reference_image=refimg, latent_strength_list=None
                )
