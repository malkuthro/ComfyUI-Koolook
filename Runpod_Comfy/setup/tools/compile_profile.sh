...
RECIPE_ID=""
RECIPES_DIR="$ROOT/setup/recipes"
OUT_DIR=""
ACTIVATE=true
IMAGE_TAG_OVERRIDE=""
...
Defaults:
  recipes-dir: Runpod_Comfy/setup/recipes
  out:         Runpod_Comfy/image/builds/<recipe-id>
  activate:    true (copies generated files to Runpod_Comfy/image/active)
EOF
}
...
[[ -z "$OUT_DIR" ]] && OUT_DIR="$ROOT/image/builds/$RECIPE_ID"
...
for n in nodes:
    ...
    target = str(n.get('target_dir', '')).strip()
    if target.startswith('/'):
        raise ValueError(f"node target_dir must be relative (not absolute): {target}")
    if target.startswith('ComfyUI/'):
        raise ValueError(f"node target_dir should be relative to custom_nodes (remove 'ComfyUI/...'): {target}")
    if '..' in pathlib.PurePosixPath(target).parts:
        raise ValueError(f"node target_dir cannot contain '..': {target}")
...
for m in models:
    ...
    dest = str(m.get('dest', '')).strip()
    if dest.startswith('/'):
        raise ValueError(f"model dest must be relative to ComfyUI root (not absolute): {dest}")
    if dest.startswith('ComfyUI/'):
        raise ValueError(f"model dest should not include 'ComfyUI/' prefix: {dest}")
    if '..' in pathlib.PurePosixPath(dest).parts:
        raise ValueError(f"model dest cannot contain '..': {dest}")
    m.setdefault('sha256',''); m.setdefault('enabled',True)

comfy_lock='\n'.join([
    '# Generated by Runpod_Comfy/setup/tools/compile_profile.sh',
