...
| [`dependency-security.md`](dependency-security.md) | Dependency CVE protocol: the locked test set as the review surface, local/CI `pip-audit` gates, relocking, and weekly scheduled audit response. |
| [`published-setup-loop.md`](published-setup-loop.md) | Architecture overview / entry point for the publish → run loop: the plain-terms loop, a technical map (frontend JS → Koolook routes → ComfyUI engine), and the contract/schema requirements. Read this first, then follow the links. |
| [`published-setups.md`](published-setups.md) | Published workflow setup schema, server-side storage boundary, sidebar publish flow, and catalog API contract for external frontends. |
| [`published-setup-external-ui-contract.md`](published-setup-external-ui-contract.md) | Product/design contract for how ComfyUI publish nodes expose controls and result fields to an external frontend. |
| [`published-setup-full-circle-plan.md`](published-setup-full-circle-plan.md) | Remaining implementation plan for the full authoring → publish → exported setup/API → external runner loop. |
| [`external-frontend-job-submitter-brief.md`](external-frontend-job-submitter-brief.md) | Handoff brief for an AI or developer session building the real external frontend job submitter. |
| [`external-frontend-kit.md`](external-frontend-kit.md) | Complete file/dependency/fixture checklist for another AI session building the production external frontend. |
...
| `preflight-release` | Runs the release-gate checks from `tools/preflight_release.py` — static AST node-ID extraction, VAE wrapper rank/type dispatch roundtrip, and workflow-fixture stability. The ComfyUI-Manager extension-node-map drift check remains available as an explicit advisory check (`--check manager-meta`) but does not block releases because Manager metadata is maintained independently. Run before opening any release PR — `releasing.md` step 1. |
